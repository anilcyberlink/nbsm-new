...
//By Sangam Starts
function removeShowInHome($data)
{
    if ($data->count() > 3) {
        $latestItems = $data->sortByDesc('updated_at')->take(3);

        // Update all items
        foreach ($data as $item) {
            // Check if this item is in the latest 3
            $item->show_in_home = $latestItems->contains('id', $item->id) ? '1' : '0';
            $item->save();
        }
    }
}

function getPosttypePostPostchildShowInHomeById($id)
{

    $data = PostModel::where(['post_type' => $id, 'show_in_home' => '1'])->where('post_parent', '!=', '0')
...
function getPosttypePostPostchildShowInHomeByIdWithStatus($id)
{

    $data = PostModel::where(['post_type' => $id, 'show_in_home' => '1', 'status' => '1'])->where('post_parent', '!=', '0')
        ->orderByRaw('home_order IS NULL')
        ->orderBy('home_order', 'asc')
        ->get();
    return $data;
