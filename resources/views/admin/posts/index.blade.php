...
<section id="" class="table-layout animated fadeIn">
    ...
    <div class=""> 
        ...
        @if(Request::segment(2) == 'resources' && Request::segment(3) != '')
            <small>You can only have 3 items shown in home form 'Resources'.<br> More than 3 will automatically remove the oldest item from show in home</small>
            <small><a href="{{route('admin.resource.home.list','resources')}}">List of item shown in home</a></small>
        @endif
        <!-- recent orders table -->
        <div class="panel">
            ...
            <tr class="id{{$row->id}}">
              ...
              <td class="post_title title_hi_sh"> 
                ...
                @if(Request::segment(2) == 'resources' && Request::segment(3) != '')
                    <td class="text-center">
                        <input class="checkShowInHome" type="checkbox" name="show_in_home" data-rowid="{{$row->id}}" {{($row->show_in_home == 1)?'checked':''}} />
                    </td>
                @endif
                <?php /*?>
